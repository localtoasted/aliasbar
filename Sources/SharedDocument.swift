...
/// A small, self-contained SHA-256 implementation (FIPS 180-4), so the store's
/// revision comparison has no dependency beyond Foundation. Correctness is checked in
/// tests against the system `shasum -a 256` binary, not just fixed test vectors.
enum SHA256Digest {
    ...
    static func hash(_ data: Data) -> [UInt8] {
        ...
        while chunkStart < message.count {
            let chunk = message[chunkStart..<(chunkStart + 64)]
